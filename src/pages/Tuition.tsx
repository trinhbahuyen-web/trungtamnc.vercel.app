...
import { CheckCircle2, Copy, QrCode, RefreshCcw, Settings, Wallet, XCircle, FileText, Search, BookOpen, Calculator, Download } from 'lucide-react';
...
export default function Tuition() {
  ...
  const generatePayrollReport = async () => {
    ...
    try {
      ...
      toast('Đã tính toán xong bảng lương GV!', 'success');
  ...
  const exportMegaPayrollExcel = () => {
    ...
    XLSX.writeFile(wb, `Quyet_Toan_Luong_${formatDate(startDate).replace(/\//g,'')}_${formatDate(endDate).replace(/\//g,'')}.xlsx`);
    toast('Đã xuất Báo cáo Đa trang thành công!', 'success');
  };
      ...
      {tabMode === 'PAYROLL' && (
        <>
          <div className="card" style={{ marginBottom: '1rem', border: '2px solid #10b981' }}>
            <div className="card-header" style={{ background: '#ecfdf5', color: '#047857' }}>
              <strong>Tính toán Doanh thu Trung tâm & Quyết toán Lương GV</strong>
            </div>
            <div className="card-body">
              <div className="form-row" style={{ alignItems: 'flex-end' }}>
                ...
                <div className="form-group" style={{ flex: 1 }}>
                  <button className="btn btn-primary" style={{ width: '100%', background: '#059669', borderColor: '#059669', height: '42px', display: 'flex', justifyContent: 'center', gap: 8 }} onClick={generatePayrollReport} disabled={calculatingPayroll}>
                    ...
                    Tính Doanh thu & Lương
                  </button>
                </div>
              </div>
              <p style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: 8 }}>
                * Hệ thống tự động quét tất cả các lớp có phát sinh điểm danh trong khoảng thời gian trên.<br/>
                * <strong>Lương GV = (Tổng thu - Phí phòng - Trừ khác) - Thuế TNCN (2%)</strong>. Thuế tự động tính lại nếu Thủ quỹ sửa Phí phòng / Khác.
              </p>
            </div>
          </div>
          ...
          {payrollData.length > 0 && (
            <div className="card">
              <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
                <span>Bảng Tổng hợp Lương Giáo Viên</span>
                <button className="btn btn-secondary btn-sm" onClick={exportMegaPayrollExcel} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Download size={14} /> Xuất file Excel (Đa trang)
                </button>
              </div>
              <div className="table-wrap">
